...
| dedicated PID namespace init and complete cgroup tree | `clone3(CLONE_NEWPID | CLONE_INTO_CGROUP)`, internal PID-1 reaper, pre-dump set equality | `checkpoint` refusal test; `tests/evidence/domain-inventory.json`; `tests/evidence/checkpoint/refused-domain-inventory.json` |
...
| browser windows, tabs, selected tabs, page memory, placement | CDP and Sway tree probes in `application-reboot.nix` | `tests/evidence/{before,after}.json` |
| terminal/tmux and shell continuity | canonical 120-line scrollback hash, tmux topology, shell fixture state | retained before/after evidence |
...
| compositor IPC, restored clients, injected input, new client | Sway IPC, restored ingress consumer, post-restore foot client | `application-reboot` assertions and retained input/outer records |
