...
| terminal/tmux and shell continuity | complete canonical terminal contents (only trailing empty cells removed), 120-line scrollback hash, tmux topology, shell fixture state | retained before/after evidence contains identical 120-entry `terminal.contents`, line counts, and full-content hashes |
...
| compositor IPC, restored clients, injected input, new client | Sway IPC, supervisor-owned `input.sock` → `wtype` → Sway virtual keyboard → restored foot/read process, post-restore foot client | `application-reboot` assertions and retained verdict/outer records |
